...
import {
  ...
  ChevronLeft, ChevronRight, Store, CornerDownLeft, Plus, Trash2,
} from 'lucide-react';
...
import { apiComputeProductAnalysis, apiComputeOptions } from '@/lib/api';
...
export function ProductAnalysis({ records, selectedDate, initialAliases, readOnly }: ProductAnalysisProps) {
  ...
  const handleStartEditCategory = useCallback((orderId: string, currentCat: string) => { setEditingOrderId(orderId); setEditCategory(currentCat || ''); }, []);
  const handleSaveCategory = useCallback((orderId: string) => {
    if (!editCategory.trim()) { setManualClassification(prev => { const n = { ...prev }; delete n[orderId]; return n; }); }
    else { setManualClassification(prev => ({ ...prev, [orderId]: editCategory.trim() })); }
  ...
  useEffect(() => {
    ...
    if (flagChartType === 'pie') {
      option = { tooltip: { ...TOOLTIP_STYLE, trigger: 'item', formatter: (p: any) => `<b>${p.name}</b><br/>${p.value}单 (${p.percent}%)` }, legend: { bottom: 0, orient: 'horizontal', itemGap: 14, textStyle: { fontSize: 11, color: '#64748b' }, itemWidth: 12, itemHeight: 12 }, series: [{ type: 'pie', radius: ['40%', '72%'], center: ['50%', '42%'], avoidLabelOverlap: true, padAngle: 2, itemStyle: { borderColor: '#fff', borderWidth: 2, borderRadius: 4 }, label: { show: true, position: 'outside', fontSize: 10, fontWeight: 'bold', color: '#334155', formatter: (p: any) => { const name = p.name.length > 4 ? p.name.slice(0, 4) + '..' : p.name; return p.percent > 8 ? `${name}\n${p.value}单(${p.percent}%)` : p.percent > 3 ? `${name} ${p.percent}%` : ''; }, lineHeight: 14, alignTo: 'edge', margin: 8 }, labelLine: { length: 16, length2: 8, lineStyle: { width: 1 } }, data: sortedFlag.map(d => ({ name: d.name, value: d.value, itemStyle: { color: FLAG_COLOR_MAP[d.name] || VIVID_COLORS[0] } })) }] };
    } else {
        ...
        <>
          <Card className="brutal-card-lift rounded-xl shadow-sm overflow-hidden border-l-4 border-l-primary/30">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div><CardTitle className="text-base font-bold flex items-center gap-2"><Package className="h-4 w-4 text-primary" />产品分析</CardTitle><CardDescription className="text-xs mt-1">基于 {dateRange?.start} ~ {dateRange?.end} 的聚合数据</CardDescription></div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {metricCards.map(card => (
                  <div key={card.label} className="rounded-xl p-4 relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${card.color}10 0%, ${card.color}05 100%)`, borderLeft: `4px solid ${card.color}` }}>
                    ...
                    <p className={`mt-1 font-black tabular-nums ${card.isText ? 'text-sm line-clamp-2' : 'text-2xl'}`} style={{ color: card.color }}>{card.value}</p>
                  </div>
                ))}
              </div>
            </CardContent>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-10 gap-6 mt-6">
            <Card className="brutal-card-lift rounded-xl shadow-sm border-t-4 border-t-emerald-400/60 lg:col-span-6">
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-sm font-bold flex items-center gap-2 flex-wrap"><Package className="h-4 w-4 text-primary" />数量分布
            ...
            <Card className="brutal-card-lift rounded-xl shadow-sm border-t-4 border-t-amber-400/60 lg:col-span-4">
              ...
              <CardContent>{flagData.length === 0 ? (<div className="flex items-center justify-center h-[300px] text-muted-foreground"><p className="text-xs">无标旗数据</p></div>) : (<div ref={flagChartRef} className="w-full h-[380px]" />)}</CardContent>
            </Card>
          </div>
                          ...
                          <TableCell className="text-xs py-1.5">
                            {isEditing ? (
                              <div className="flex items-center gap-1.5">
                                ...
                                <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-green-600 hover:bg-green-50 rounded-full" onClick={() => handleSaveCategory(orderId)}><Check className="h-4 w-4" /></Button>
